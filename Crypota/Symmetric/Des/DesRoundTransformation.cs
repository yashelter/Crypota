...
public class DesRoundTransformation : IEncryptionTransformation
{
    ...
    private static (int, int) GetSBlockIndexes(Span<byte> bArray, int startPos)
    {
    ...
    /// <summary>
    /// Realization of F transformation function for Feistel Network
    /// </summary>
    /// <param name="state"></param>
    /// <param name="roundKey"></param>
    /// <returns></returns>
    public void EncryptionTransformation(Span<byte> state, Span<byte> roundKey) // function f
    {
        var eRi = PermuteBits(state, ETable, 1);
        var bBlocks = XorTwoParts(eRi, roundKey);
        ...
        var pAffected = PermuteBits(bAffected, PTable, 1);
        pAffected.CopyTo(state);
    }
