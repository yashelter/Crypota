...
public class CfbHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        byte[] feedbackRegister = ArrayPool<byte>.Shared.Rent(blockSize);
        byte[] currentCiphertext = ArrayPool<byte>.Shared.Rent(blockSize);

        try
        {
            iv.CopyTo(feedbackRegister.AsSpan(0, blockSize));
            Span<byte> feedbackSpan = feedbackRegister.AsSpan(0, blockSize);
            Span<byte> tempCiphertextSpan = currentCiphertext.AsSpan(0, blockSize);
            ...
            for (int i = 0; i < totalBlocks; i++)
            {
                ...
                currentBlock.CopyTo(tempCiphertextSpan);
                encryptor.EncryptBlock(feedbackSpan);
        ...
        {
            ArrayPool<byte>.Shared.Return(feedbackRegister);
            ArrayPool<byte>.Shared.Return(currentCiphertext);
        }
    }
